namespace ExactOnline.Client.Sdk.Helpers
{
    ...
    public class ExactOnlineQuery<T>
    {
        ...
        /// </summary>
        public ExactOnlineQuery<T> Where<TProperty>(Expression<Func<T, TProperty>> property, TProperty value,
            OperatorEnum @operator = OperatorEnum.Eq)
        {
            return Where(
                $"{TransformExpressionToODataFormat(property.Body)}+{@operator.ToString().ToLower()}+{ToODataParameter(value)}");
        }
        ...
        /// </summary>
        public ExactOnlineQuery<T> And<TProperty>(Expression<Func<T, TProperty>> property, TProperty value,
            OperatorEnum @operator = OperatorEnum.Eq)
        {
            return And(
                $"{TransformExpressionToODataFormat(property.Body)}+{@operator.ToString().ToLower()}+{ToODataParameter(value)}");
        ...
        private string CreateODataQuery(bool selectIsMandatory)
        {
            ...
            if (!string.IsNullOrEmpty(_where))
            {
                ...
                {
                    _where += $"+and+{string.Join("+and+", _and)}";
                }
        ...
        {
            return Select(property.Select(x => TransformExpressionToODataFormat(x.Body)).ToArray());
        }
        ...
        private ExactOnlineQuery<T> FormulateSkipToken(string skipToken)
        {
            ...
            {
                _skipToken = $"$skiptoken={skipToken}";
            }
        ...
        /// </summary>
        /// <param name="skipToken">The variable to store the skiptoken in</param>
        public Task<ApiList<T>> GetAsync(string skiptoken = "")
        {
            FormulateSkipToken(skiptoken);
        ...
        private string TransformExpressionToODataFormat(Expression e)
        {
            ...
            if (mce == null)
            {
                throw new ArgumentException(
                    $"Invalid expression '{e}': Lambda expression should resolve a property on model type '{nameof(T)}' (with optional extension method calls).",
                    nameof(e));
            }
        ...
        /// </summary>
        private string ToODataParameter(object value)
        {
            string _value = null;
