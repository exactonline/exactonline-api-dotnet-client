namespace ExactOnline.Client.Sdk.Helpers
{
    ...
    public class ExactOnlineJsonConverter : JsonConverter
    {
        ...
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            ...
            var guidsToSkip = writeableFields
                .Where(x => x.GetValue(value) is Guid && (Guid)x.GetValue(value) == Guid.Empty).ToArray();

            // Remove the fields to skip from the writeable fields
            writeableFields = writeableFields
                .Except(writeableFields.Join(guidsToSkip, e => e.GetValue(value), m => m.GetValue(value), (e, m) => e))
                .ToArray();
        ...
        {
            var attributes = pi.GetCustomAttributes().Where(x => x.GetType() == typeof(SDKFieldType)).ToArray();
        ...
        {
            return _entityControllerDelegate(entity);
        }
        ...
        private PropertyInfo[] GetWriteableFields(object value, Boolean jsonForUpdate)
        {
            ...
            {
                var updatedfields =
                    GetUpdatedFields(writeableFields, value); // If Json for update: Get only updated fields
