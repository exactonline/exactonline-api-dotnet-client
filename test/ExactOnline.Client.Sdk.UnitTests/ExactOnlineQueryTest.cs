namespace ExactOnline.Client.Sdk.UnitTests
{
    ...
    {
        private Controller<Account> _acccountController;
        private ApiConnectionMock _conn;
        private ControllerMock<Account> _controllerMock;
        ...
        public void ExactOnlineQuery_TestAnd_WithCorrectString_Succeeds()
        {
            ...
            Assert.AreEqual("$filter=Name+eq+'Test Testname'+and+Code+eq+'123'+and+Code+eq+'456'&$select=Code,Name",
                _controllerMock.ODataQuery);
        }
        ...
        {
            var newAccount = new Account();
            var insertedAccount = await new ExactOnlineQuery<Account>(_controllerMock).InsertAsync(newAccount)
                .ConfigureAwait(false);
        ...
        {
            await new ExactOnlineQuery<Account>(_controllerMock).Top(10).Select("Code", "Name").GetAsync()
                .ConfigureAwait(false);
            const string expected = "$select=Code,Name&$top=10";
            var query = _controllerMock.ODataQuery;
            Assert.AreEqual(expected, query);
        ...
        public void ExactOnlineQuery_WithAllOptions_Succeeds()
        {
            ...
            const string expected =
                "$filter=Name+eq+'Test'+and+Name+eq+'Test2'&$select=Description,Name&$skip=10&$expand=BankAccounts&$top=10";
        ...
        public async Task ExactOnlineQuery_WithAllOptions_SucceedsAsync()
        {
            ...
            const string expected =
                "$filter=Name+eq+'Test'+and+Name+eq+'Test2'&$select=Description,Name&$skip=10&$expand=BankAccounts&$top=10&$skiptoken=mySkiptoken";
            var data = _controllerMock.ODataQuery;
            Assert.AreEqual(expected, data);
        }
    }
